...
public class VSSlider: UIControl {
    ...
    @objc func sliderTapped(gestureRecognizer: UIGestureRecognizer) {
        ...
        let pointTapped: CGPoint = gestureRecognizer.location(in: slider)
                ...
                // Calculate the value based on the tap location
                let value = pointTapped.x / slider.bounds.size.width
                let updatedValue = minimumValue + Float(value) * (maximumValue - minimumValue)
    ...
    // MARK: - Slider Pan Gesture Handling
    ...
    fileprivate func initialize() {
       ...
       //  Add a gesture recognizer to the slider
        ...
        // Add a pan gesture recognizer to track continuous touch movement
    ...
    private func getRangeForMarks() -> [Float] {
        ...
        // exclude Center Mark
        if let index = range.firstIndex(of: (maximumValue + minimumValue) / 2) {
